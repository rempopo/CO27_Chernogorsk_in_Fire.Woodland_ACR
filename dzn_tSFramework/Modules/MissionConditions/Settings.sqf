...
MissionCondition1 = [ "WIN1", {
	...
	&& CIV2 inArea baseTrg 
	&& {call fnc_CheckPlayersReturned} 
}, "2 vip saved, 2 civ saved" ];

MissionCondition2 = [ "WIN2", {
	VIP1 inArea baseTrg
	&& VIP2 inArea baseTrg 
	&& (!alive CIV1 && !alive CIV2)
	&& {call fnc_CheckPlayersReturned} 
}, "2 vip saved, 2 civ dead" ];
...
MissionCondition3 = [ "WIN3", {
	...
	&& CIV2 inArea baseTrg 
	&& {call fnc_CheckPlayersReturned}
}, "1 vip saved, 1 vip dead, 2 civ saved" ];
						
MissionCondition4 = [ "WIN4", {
	(( alive VIP1 && !alive VIP2 && VIP1 inArea baseTrg )
	|| ( alive VIP2 && !alive VIP1 && VIP2 inArea baseTrg )
	) 	&& (!alive CIV1 && !alive CIV2)
	&& {call fnc_CheckPlayersReturned}
}, "1 vip saved, 1 vip dead, 2 civ dead" ];

MissionCondition5 = [ "WIN5", {
     (!alive VIP1 && !alive VIP2)
	&& (!alive CIV1 && !alive CIV2)
	&& {call fnc_CheckPlayersReturned} 
}, "2 vip dead, 2 civ dead" ];

MissionCondition6 = [ "WIN6", {
	(( alive VIP1 && !alive VIP2 && VIP1 inArea baseTrg )
	|| ( alive VIP2 && !alive VIP1 && VIP2 inArea baseTrg )
	) 	&& (( alive CIV1 && !alive CIV2 && CIV1 inArea baseTrg )
	|| ( alive CIV2 && !alive CIV1 && CIV2 inArea baseTrg )
	)    && {call fnc_CheckPlayersReturned}
}, "1 vip saved, 1 vip dead, 1 civ dead, 1 civ saved" ];

MissionCondition7 = [ "WIN7", {
     (!alive VIP1 && !alive VIP2)
	&& (( alive CIV1 && !alive CIV2 && CIV1 inArea baseTrg )
	|| ( alive CIV2 && !alive CIV1 && CIV2 inArea baseTrg )
	)    && {call fnc_CheckPlayersReturned}
}, "2 vip dead, 1 civ dead, 1 civ saved" ];

MissionCondition8 = [ "WIN9", {
     (!alive VIP1 && !alive VIP2)
	&& CIV1 inArea baseTrg 
	&& CIV2 inArea baseTrg 
	&& {call fnc_CheckPlayersReturned} 
}, "2 vip dead, 2 civ saved" ];
						
MissionCondition9 = [ "WIPED", { call fnc_isAllDead }, "All dead", 30 ];